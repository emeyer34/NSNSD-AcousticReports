...
params:
  ...
  year:
    ...
    sep: ""
  order:
    label: "What order did this season and year monitoring combo occur? (must be increments of 1)"
    value: 1
    input: slider
    min: 0
    max: 10
  ...
  plotCONTOUR:
    ...
    choices: [TRUE, FALSE]
  yMaxHr:
    label: "What's the upper limit for the hourly graphs? (must be increments of 3)"
    value: 70
    input: slider
    min: 50
    max: 90
    ...
    sep: ""
  yMinHr:
    label: "What's the lower limit for the hourly graphs? (must be increments of 3)"
    value: 5
    input: slider
    min: 2
    max: 15
    ...
    sep: ""
  yMaxHz:
    label: "What's the upper limit for the frequency graphs? (must be increments of 3)"
    value: 70
    input: slider
    min: 50
    max: 90
    ...
    sep: ""
  yMinHz:
    label: "What's the lower limit for the frequency graphs? (must be increments of 3)"
    value: -12
    input: slider
    min: -24
    max: 24
...
dir.create(file.path(report_dir, paste0(params$order,"_",params$season, "_", params$year)))
...
for (i in seq_along(dflist)) {
  ...
  dflist[[i]] <- cbind("Site ID" = names[i], "Frequency (Hz)" = r"(ANS (20-1,250))", ansday, ansnight)
}
...
write.csv(exectab, 
          file = paste0(report_dir, "/", params$order,"_",params$season, "_", params$year, "/executivesumtab_", params$season, ".csv"), 
...
write.csv(amb_full_all, 
          file = paste0(report_dir, "/", params$order,"_",params$season, "_", params$year, "/ambfullsum_", params$season, ".csv"),
          row.names = FALSE)  # Added row.names = FALSE for cleaner output
...
write.csv(timeAbove, 
          file = paste0(report_dir, "/", params$order,"_",params$season, "_", params$year, "/timeabove_", params$season, ".csv"), 
          row.names = FALSE)  # Added row.names = FALSE for cleaner output
...
write.csv(impact_la, 
          file = paste0(report_dir, "/", params$order,"_",params$season, "_", params$year, "/impactlisteningarea_", params$season, ".csv"), 
          row.names = FALSE)  # Added row.names = FALSE for cleaner output
```
...
for (i in seq_along(newnames)){
  ...
  write.csv(data, 
            file = paste0(report_dir, "/", params$order,"_",params$season, "_", params$year, "/ListeningCenter_", 
...
for (i in seq_along(newnames2)) {
  ...
  write.csv(data, 
            file = paste0(report_dir, "/", params$order,"_",season, "_", params$year, "/SPLAT_", 
...
outDir <- paste(report_dir, "/", params$order,"_",params$season, "_", params$year, sep = "")
...
```{r all source graph detail,echo=FALSE, eval=eitherfiles, fig.width=8, fig.height=5, warning=FALSE,message=FALSE}
# If SPLAT data exists, it will combine with Listening Center data; otherwise, it uses just Listening Center data
detcombined <- rbind(detailfinal, if (exists("splatdetailfinal")) splatdetailfinal)
detcombined <- merge(detcombined,sourceid, by= "Source Description")
# Average the 24-hour data into one column
detcombined$dailyave <- rowMeans(detcombined[, 3:26], na.rm = TRUE)  # Added na.rm to handle potential NAs
...
# Take source description data and obtain 24-hour averages for source detail categories and source types
# Data frame is for percentages of different broad source types used in executive table summary
...
for (i in 1:length(names)) {
  ...
  # Create the plot
  plot <- ggplot(data = df1, aes(x = mean, y = `Source Description`, fill = SourceCategory)) +
    geom_col(width = 0.9) +
  ...
  ggsave(paste(names[i], "_", params$season, "_PercentAud_description_all.png", sep = ""), 
...
``` {r all source graph cat, echo=FALSE, eval=eitherfiles, fig.width=8, fig.height=5, warning=FALSE,message=FALSE}
...
for (i in 1:length(names)) {
  ...
  # Save the plot with a dynamic filename based on SiteID and other parameters
  ggsave(paste(names[i], "_", params$season, "_PercentAud_category_all.png", sep = ""), 
         plot = plot, height = 5, width = 8, path = outDir)
}
...
### Audibility (All Sources-Categories)


```{r, results='asis',fig.cap=paste("Your caption."), echo=FALSE, include=TRUE, warning=FALSE, message=FALSE}

# List all PNG files with the pattern "PercentAud_top2"
audpngfiles <- list.files(path = outDir, pattern = "PercentAud_category_all", full.names = TRUE)
...
### Audibility (All Sources-Detailed Description)


```{r, results='asis',fig.cap=paste("Your caption."), echo=FALSE, include=TRUE, warning=FALSE, message=FALSE}

# List all PNG files with the pattern "PercentAud_top2"
audpngfiles <- list.files(path = outDir, pattern = "PercentAud_description_all", full.names = TRUE)
